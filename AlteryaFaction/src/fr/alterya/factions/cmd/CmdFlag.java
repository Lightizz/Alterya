...
import fr.alterya.factions.zcore.Lang;
...
public class CmdFlag extends FCommand
{
	...
	public void perform()
	{
		...
		if ( ! this.argIsSet(1))
		{
			msg(p.txt.titleize("Flags pour " + faction.describeTo(fme, true)));
		...
		if ( ! this.argIsSet(2))
		{
			msg(p.txt.titleize("Flag pour " + faction.describeTo(fme, true)));
			msg(flag.getStateInfo(faction.getFlag(flag), true));
			return;
		}
